...
void* l_append(void* lst, void* stud){
    struct List* list = (struct List*)lst;
    struct Studentik* student = (struct Studentik*)stud;
    ...
    if(list->SIZE == 0) {
        ...
        list->tl->next = student;
        list->tl = student;
    }
    list->SIZE++;

    return NULL; // No meaningful return value for this function
}

void l_print(void* lst){
    struct List* list = (struct List*)lst;
